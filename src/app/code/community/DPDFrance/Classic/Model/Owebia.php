...
//                 Mage::getModel('core/log_adapter')->log("classic disabled");
...
//                     Mage::getModel('core/log_adapter')->log("classic dosen't match");
                    return new DPDFranceClassic_Os_Result(false);
                }
            }
        }
...
//                 Mage::getModel('core/log_adapter')->log("classic no cover zone");
                return new DPDFranceClassic_Os_Result(false);
            }
        }
//         Mage::getModel('core/log_adapter')->log("process['data']['shipto.postal.code']: ".$process['data']['shipto.postal.code']);#ok
...
//         Mage::getModel('core/log_adapter')->log("classic process['data']['origin.postal.code']: ".$process[data][origin.postal.code]);#nok
        $zipcodeStore = Mage::getStoreConfig('dpdfrexport/embarcador/cep');
//         Mage::getModel('core/log_adapter')->log("classic zipcodeStore: $zipcodeStore");
        ...
        $preco = Mage::helper('dpdfrexport')->getPrecoFrete($zipcodeStore, $process['data']['shipto.postal.code'], $vlDec, $peso, $cpfOrCnpj, '9');
        if ($preco == false) {
            $this->addMessage('info',$row,'customer_groups',"Resposta inválida do servidor de frete",$process['data']['customer.group.code']);
            return new DPDFranceClassic_Os_Result(false);
        }
        $precoAux=str_replace('.','', (string) $preco);
        $precoAux=str_replace(',','.',$precoAux);
//         Mage::getModel('core/log_adapter')->log("classic preco: $preco");
//         Mage::getModel('core/log_adapter')->log("classic precoAux: $precoAux");
//         Mage::getModel('core/log_adapter')->log("(float) precoAux: ".$precoAux);
        return new DPDFranceClassic_Os_Result(true, $precoAux);
...
//             Mage::getModel('core/log_adapter')->log("classic result->result: $result->result");
...
//         return new DPDFranceClassic_Os_Result(false);
    ...
    protected function _addressMatch($address_filter, $address) {
        // fazendo retornar sempre que tem cobertura
        return true;
