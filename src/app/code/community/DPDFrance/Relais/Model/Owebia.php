...
class DPDFrance_Relais_Model_Owebia
{
    ...
    {
        if ($this->debug > 0)
            echo "<style rel=\"stylesheet\" type=\"text/css\">" . ".osh-formula{color:#f90;} .osh-key{color:#0099f7;}" . ".osh-error{color:#f00;} .osh-warning{color:#ff0;} .osh-info{color:#7bf700;}" . ".osh-debug{background:#000;color:#bbb;position:absolute;top:0;left:0;width:100%;z-index:100;-moz-opacity:0.9;opacity:0.9;text-align:left;white-space:pre-wrap;}" . ".osh-debug-content{padding:10px;}" . ".osh-replacement{color:#ff3000;}" . "</style>" . "<div id=\"osh-debug\" class=\"osh-debug\"><pre class=\"osh-debug-content\"><span style=\"float:right;cursor:pointer;\" onclick=\"document.getElementById('osh-debug').style.display = 'none';\">[<span style=\"padding:0 5px;color:#f00;\">X</span>]</span>" . "<p>" . $this->debug_header . "</p>" . $this->debug_output . "</pre></div>";
    }
    ...
    {
        if (! isset($row['*code'])) {
//             $logger = Mage::getModel('core/log_adapter')->log("nao esta ajustado a row");
            return;
        }
...
//                 $logger = Mage::getModel('core/log_adapter')->log("is not enabled");
...
//                     $logger = Mage::getModel('core/log_adapter')->log("nao esta em condicoes");
                    return new DPDFranceRelais_Os_Result(false);
                }
            }
        }
...
//                 $logger = Mage::getModel('core/log_adapter')->log("area descoberta");
                return new DPDFranceRelais_Os_Result(false);
            }
        }
...
//                 $logger = Mage::getModel('core/log_adapter')->log("origem descoberta");
                return new DPDFranceRelais_Os_Result(false);
            }
        }
...
//                 $logger = Mage::getModel('core/log_adapter')->log("grupo do usuario descoberto");
                return new DPDFranceRelais_Os_Result(false);
            }
        }
        $zipcodeTo = $process['data']['shipto.postal.code'];
        $zipcodeStore = Mage::getStoreConfig('dpdfrexport/embarcador/cep');
        
        $vlDec = $process['data']['cart.price_including_tax'];
        $peso = $process['data']['peso_taxado'];
        $cpfOrCnpj = $process['data']['cpfOrCnpj'];
        $preco = Mage::helper('dpdfrexport')->getPrecoFrete($zipcodeStore, $zipcodeTo, $vlDec, $peso, $cpfOrCnpj, '40');
        if ($preco == false) {
            $this->addMessage('info',$row,'customer_groups',"Resposta inválida do servidor de frete",$process['data']['customer.group.code']);
            return new DPDFranceClassic_Os_Result(false);
        }
        $preco = (float) str_replace(",", ".", (string) $preco);
        return new DPDFranceRelais_Os_Result(true, $preco);
    }
    ...
    public function getRowProperty($row, $key, $original_row = null, $original_key = null)
    {
        ...
        if (isset($row[$key]['value'])) {
            ...
            $output = $property;
            $code = array_key_exists('*code', $row) ? $row['*code'] : NULL;
            self::debug('   get <span class="osh-key">' . $code . '</span>.<span class="osh-key">' . $key . '</span> = <span class="osh-formula">' . $this->_toString($property) . '</span>', 5);
            preg_match_all('/{([a-z0-9_]+)\.([a-z0-9_]+)}/i', $output, $result_set, PREG_SET_ORDER);
            foreach ($result_set as $result) {
                list ($original, $ref_code, $ref_key) = $result;
                if (! in_array($ref_code, array(
                    'module',
                    'date',
                    'store',
                    'cart',
            ...
            $code = array_key_exists('*code', $row) ? $row['*code'] : NULL;
            self::debug('   get <span class="osh-key">' . $code . '</span>.<span class="osh-key">' . $key . '</span> = <span class="osh-formula">null</span>', 5);
        }
        return $output;
    }
    ...
    protected function _prepareFormula($process, $row, $property_key, $formula_string, $is_checking, $use_cache = true)
    {
        ...
        if (isset($first_product)) {
            while (preg_match("#{product\.(attribute|option|stock)\.([a-z0-9_]+)}#i", $formula, $result)) {
                ...
                switch ($result[1]) {
                    case 'attribute':
                        $replacement = $first_product->getAttribute($result[2]);
                        break;
                    case 'option':
                        $replacement = $first_product->getOption($result[2]);
                        break;
                    case 'stock':
                        $replacement = $first_product->getStockData($result[2]);
                        break;
                }
                $formula = $this->replace($original, $replacement, $formula);
            }
    ...
    protected function _evalFormula($formula)
    {
        if (is_bool($formula))
            return $formula;
        if (! preg_match('/^(?:floor|ceil|round|max|min|rand|pow|pi|sqrt|log|exp|abs|int|float|true|false|null|and|or|in|substr|strtolower' . '|in_array\(\'(?:[^\']*)\', *array\( *(?:\'(?:[^\']+)\') *(?: *, *\'(?:[^\']+)\')* *\) *\)' . '|\'[^\']*\'|[0-9,\'\.\-\(\)\*\/\?\:\+\<\>\=\&\|%! ])*$/', $formula)) {
    ...
    public function compress($input)
    {
        ...
        return '$$' . str_replace(self::$UNCOMPRESSED_STRINGS, self::$COMPRESSED_STRINGS, $input);
    }
    ...
    public function uncompress($input)
    {
        if (preg_match_all("/{table (.*) in (" . self::$COUPLE_REGEX . "(?:, *" . self::$COUPLE_REGEX . ")*)}/iU", $input, $result, PREG_SET_ORDER)) {
            foreach ($result as $result_i) {
                ...
                foreach ($fees_table as $index => $item) {
                    ...
                    $fees_table[$index] = (((float) $new_max_value) == $max_value ? $new_max_value : $max_value) . $including_char . ':' . (((float) $new_fee) == $fee ? $new_fee : $fee);
                }
                $input = str_replace($result_i[2], implode(', ', $fees_table), $input);
                $input = str_replace($result_i[1], trim($result_i[1]), $input);
            }
        ...
        return str_replace(self::$COMPRESSED_STRINGS, self::$UNCOMPRESSED_STRINGS, $input);
    }
        ...
        if (is_string($input)) {
                ...
                while (preg_match('/' . $regex . '/', $input, $resi)) {
                    switch ($resi[1]) {
                        ...
                        case 'products_quantity':
                            $input = str_replace($resi[0], "{cart.quantity}", $input);
                            break;
                        case 'country':
                            $input = str_replace($resi[0], "{shipto.country.name}", $input);
                            break;
                    }
                }
            }
            ...
            if (preg_match('/' . $regex1 . '/', $input, $resi) || preg_match('/' . $regex2 . '/', $input, $resi)) {
                ...
                while (preg_match('/' . $regex1 . '/', $input, $resi) || preg_match('/' . $regex2 . '/', $input, $resi)) {
                    switch ($resi[1]) {
                        case 'count':
                            $input = str_replace($resi[0], "{count products where product." . $resi[2] . "s." . $resi[3] . $resi[4] . "}", $input);
                            break;
                        case 'all':
                            $input = str_replace($resi[0], "{count products where product." . $resi[2] . "s." . $resi[3] . $resi[4] . "}=={products_quantity}", $input);
                            break;
                        case 'any':
                            $input = str_replace($resi[0], "{count products where product." . $resi[2] . "s." . $resi[3] . $resi[4] . "}>0", $input);
                            break;
                        case 'sum':
                            $input = str_replace($resi[0], "{sum product." . $resi[2] . "." . $resi[3] . "}", $input);
                            break;
                    }
                }
            }
        ...
        foreach ($config as $code => $object) {
            ...
            foreach ($object as $property_name => $property_value) {
                if (in_array($property_name, $reserved_keys))
                    continue;
                if (in_array($property_name, $available_keys) || substr($property_name, 0, 1) == '_' || in_array($object['type'], array(
                    'data',
                    'meta'
                ))) {
                ...
                } else if (! in_array($property_name, $unknown_properties))
    ...
    {
        // esta sempre dando descoberto retirei o filtro
        return true;
    ...
    protected function _getProductProperty($product, $property_type, $property_name, $get_by_id = false)
    {
        switch ($property_type) {
            case 'attribute':
            case 'attributes':
                return $product->getAttribute($property_name, $get_by_id);
            case 'option':
            case 'options':
                return $product->getOption($property_name, $get_by_id);
            case 'stock':
                return $product->getStockData($property_name);
        }
        return null;
    }
        ...
        foreach ($products as $product) {
            if (isset($conditions) && $conditions != '') {
                ...
                foreach ($properties as $property) {
                    $value = $this->_getProductProperty($product, $tmp_property_type = $property[1], $tmp_property_name = $property[2], $get_by_id = isset($property[3]) && $property[3] == 'id');
